  ...
  <v-card flat>
    ...
    <v-card-text>
      <v-combobox
        ...
        full-width
        open-on-clear
        :search-input.sync="exSearchInput"
        @change="exSearchInput = ''"
      >
      </v-combobox>
      <v-checkbox
        class="mt-0 pt-0"
...
export default {
  ...
  computed: {
    ...
    examination() {
      return this.$store.getters[`${TEMPLATES_NAMESPACE}/examination`].map(
        item => {
          return item.content;
        }
      );
  ...
  created() {
    this.loading = true;
    this.$store.dispatch(`${TEMPLATES_NAMESPACE}/getExamination`).then(() => {
      this.loading = false;
    });
  }
};
