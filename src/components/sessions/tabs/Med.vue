    ...
    <v-card-text>
      <v-simple-table fixed-header max-height="600px" class="cTable">
        <template v-slot:default>
          ...
          <tbody>
            <tr v-for="(item, index) in medicines" :key="item.medicine + index">
            ...
            >
              ...
              <td colspan="5" class="pa-0">
                <v-combobox
                  ...
                  open-on-clear
                  background-color="transparent"
                  :search-input.sync="doseSearchInput"
                  @change="doseSearchInput = ''"
                >
                </v-combobox>
              </td>
              <td colspan="2">
        ...
        <v-form ref="form" v-model="valid" v-show="toggle" lazy-validation>
          ...
          <v-row>
            ...
            >
              ...
              <v-date-picker
                v-model="selectedDate"
                :value="selectedDate || new Date().toISOString().substr(0, 10)"
                @input="menu2 = false"
              ></v-date-picker>
            </v-menu>
          </v-row>
...
export default {
  ...
  computed: {
    ...
    medicines: {
      get() {
        return this.$store.getters[`${SESSION_NAMESPACE}/medicines`] || [];
      },
      set(val) {
    ...
    name: {
      get() {
        return this.$store.getters[`${SESSION_NAMESPACE}/userData`]["name"];
      },
      set(val) {
    ...
    age: {
      get() {
        return this.$store.getters[`${SESSION_NAMESPACE}/userData`]["age"];
      },
      set(val) {
        this.$store.commit(`${SESSION_NAMESPACE}/userData`, {
          name: "age",
    ...
    gender: {
      get() {
        return this.$store.getters[`${SESSION_NAMESPACE}/userData`]["gender"];
      },
      set(val) {
        this.$store.commit(`${SESSION_NAMESPACE}/userData`, {
          name: "gender",
  ...
  data() {
    return {
      ...
      nameRules: [
        ...
        v => (!!v && v.length > 1) || "field must be at least 2 characters"
      ],
      ageRules: [
        v => {
          return v
