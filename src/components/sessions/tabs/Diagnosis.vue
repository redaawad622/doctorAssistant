            ...
            >
              <td colspan="8" class="pa-0">
                <v-combobox
                  ...
                  :search-input.sync="diSearchInput"
                  @change="diSearchInput = ''"
                >
                </v-combobox>
              </td>
              <td colspan="2">
...
export default {
  computed: {
    ...
    diagnosiss: {
      get() {
        return this.$store.getters[`${SESSION_NAMESPACE}/diagnosis`] || [];
      },
      set(val) {
  ...
  created() {
    this.loading = true;
    this.$store.dispatch(`${TEMPLATES_NAMESPACE}/getDiagnosis`).then(() => {
      this.loading = false;
    });
  }
};
